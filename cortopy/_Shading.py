...
class Shading:
    ...
    NODE_TYPES = {
        ...
        "brght_contrast_node":"ShaderNodeBrightContrast",
    ...
    def node(
        ...
        tree_or_material: bpy.types.Material, 
        location=(0, 0)
        ):
        ...
        Args:
            key_or_bl_idname (str): key in the NODE_TYPES dict or Blender node type identifier (e.g., "ShaderNodeBsdfDiffuse")
            tree_or_material: bpy.types.Material OR bpy.types.NodeTree
            location (tuple): location in the node tree. Defaults to (0, 0).
    ...
    def texture_node(
        ...
        location:tuple=(0, 0)):
        """Method to create an image texture node
    ...
    def link_nodes(
        cls, 
        tree_or_material: bpy.types.Material, 
        node_output: bpy.material.node_tree.nodes, 
        node_input: bpy.material.node_tree.nodes
        ):
    ...
    def create_new_material(
            name: str, 
            displace_and_bump:str = None
            ):
        """Creata a new material
        ...
        if displace_and_bump is not None:
            material.displacement_method = displace_and_bump #TODO: Does this really affect the final material properties?
        return material

    def print_material_node_tree(material:bpy.types.Material):
    ...
    def create_simple_diffuse_BSDF(
        ...
        """
        # Part 1 - Create nodes
        material_node = Shading.material_output(material, (200, 0))
        diffuse_BSDF = Shading.diffuse_BSDF(material, (0, 0))

        # Part 2 - Setup properties
        diffuse_BSDF.inputs["Color"].default_value = (
            BSDF_color_RGB[0], 
            BSDF_color_RGB[1],
            BSDF_color_RGB[2], 
            1)
        ...
        # Part 3 - Link nodes
        Shading.link_nodes(material, diffuse_BSDF.outputs['BSDF'], material_node.inputs['Surface'])

    def create_simple_PBSDF(
        material:bpy.data.materials,
        ...
        ior:float=180,
        coat_weight:float=0,
        coat_roughness:float=1,
        coat_tint=np.array([1, 0, 0]),
    ):
        """Generate a simple Principled BSDF shader.

        Args:
            material (bpy.data.materials): material 
            color_RGB (np.array, optional): color value. Defaults to np.array([1, 0, 0]).
            metallic (float, optional): metallic value. Defaults to 0.
            roughness (float, optional): roughness value. Defaults to 1.
            ior (float, optional): ior value. Defaults to 180.
            coat_weight (float, optional): _description_. Defaults to 0.
            coat_roughness (float, optional): coat weight value. Defaults to 1.
            coat_tint (np.array, optional): coat tint value. Defaults to np.array([1, 0, 0]).
        """
        ...
        shader_node.inputs["IOR"].default_value = ior
        shader_node.inputs["Coat Weight"].default_value = coat_weight
        shader_node.inputs["Coat Roughness"].default_value = coat_roughness
    ...
    def create_oren_nayar_shader(
        ...
        ## Part 3 - Link nodes toghether
        Shading.link_nodes(material, diffuse_BSDF_node.outputs["BSDF"], material_node.inputs["Surface"]) # Diffuse BSDF to output
        ...
        if albedo_path_key in state.path:
            ...
            Shading.link_nodes(material,albedo_texture.outputs["Color"],diffuse_BSDF_node.inputs["Color"])
    ...
    def create_branch_albedo_mix(
        material: bpy.types.Material, 
        state: State, 
        settings:dict = None , 
        id_body: int = None
        ):
        """Method to create a shading tree with albedo texture and mix shaders

        Args:
            material (bpy.data.materials): material in which the node tree is generated
            state (corto.State): State object, used for paths handling
            settings (dict): Dictionary with settings for this particular shader
            id_body (int): extra input for multiple bodies case
    ...
    def create_branch_albedo_and_displacement_mix(
        material:bpy.types.Material, 
        state: State, 
        settings:dict = None, 
        id_body: int = None
    ...
    def create_randomized_texturePBSDF(
        ...
        """Generate a randomized procedural Principled BSDF shader and assign it to the given Blender material.

            This function builds a node-based shader composed of Wave, Magic,
            and Noise procedural textures, mixes them together, and connects
            the result to a Principled BSDF node. It allows fine control over
            both texture structure and material optical properties.

        Args:
            material (bpy.data.materials): material 
            wave_scale (float): Scale of the Wave texture.
            wave_distortion (float): Distortion applied to the Wave texture.
            wave_detail (float): Detail level of the Wave texture.
            wave_detail_scale (float): Scale multiplier for Wave texture detail.
            wave_detail_roughness (float): Roughness of Wave texture detail.
            magic_scale (float): Scale of the Magic texture.
            magic_distortion (float): Distortion strength of the Magic texture.
            noise_scale (float): Scale of the Noise texture.
            noise_detail (float): Detail level of the Noise texture.
            noise_roughness (float): Roughness of the Noise texture.
            noise_lacunarity (float): Lacunarity parameter controlling octave frequency scaling.
            noise_distortion (float): Distortion applied to the Noise texture.
            mix_1_fac (float): Mix factor between the first pair of procedural textures.
            mix_2_fac (float): Mix factor for the second mixing stage.
    ...
    def update_randomized_texturePBSDF(
            material:bpy.types.Material, 
            settings:dict
            ):
        """Update properties of a randomizd PBSDF shader.
           This functions works for a shader created using the Shading.create_randomized_texturePBSDF method 
           and updates the properties of the procedural textures and mix nodes according to the input settings.
    ...
    def create_earth_shader(
        ...
        """Generate Earth-based PBSDF shader. 
           This shader contains clouds, land-ocean map, nightlight, and atmospheric effects
    ...
    def create_lunar_shader(
        ...
        """This function creates a material shader using Open Shading Language (OSL) for tiles of the lunar surface.
           It loads external OSL disk and phase function scripts, configures their
           parameters, and connects them within the material node tree. The shader
           is intended for physically-based surface scattering models (e.g., custom
           reflectance or photometric models).

        Args:
        material (bpy.types.Material): The Blender material to which the OSL shader will be applied.
            The material must have `use_nodes = True` and the render engine
            must support OSL (e.g., Cycles with OSL enabled).
        state (corto.State): CORTO state object, used for handling file paths and other contextual information.
        function (str): Name or identifier of the reflectance/scattering model to use.
            This typically selects which OSL logic or BRDF formulation
            is applied inside the shader.
        geometric_albedo (float): Geometric albedo of the surface. Controls overall reflectance
            scaling of the material.
        disk_function_path (str): File path to the OSL script implementing the disk (surface)
            scattering function.
        phase_function_path (str): File path to the OSL script implementing the phase function
            (angular scattering behavior).
        osl_coeffs (dict, optional): Dictionary of additional coefficients or parameters to pass
            to the OSL shader. Keys should match parameter names defined
            in the OSL scripts. Defaults to None.
        """
    ...
    def create_osl_group(
        ...
        """This function creates a group shader using Open Shading Language (OSL).
           It loads external OSL disk and phase function scripts, configures their
           parameters, and connects them within the material node tree. The shader
           is intended for physically-based surface scattering models (e.g., custom
           reflectance or photometric models).
    ...
    def create_osl_shader(
        ...
        """This function creates a material shader using Open Shading Language (OSL).
           It loads external OSL disk and phase function scripts, configures their
           parameters, and connects them within the material node tree. The shader
           is intended for physically-based surface scattering models (e.g., custom
           reflectance or photometric models).
    ...
    def update_osl_geometry(
        ...
        Args:
            material (bpy.types.NodeTree): Material
            settings (dict): Dictionary with settings for the update
        """
    ...
    def create_phobos_opt_shader(
        ...
        value_node.outputs[0].default_value = settings['base_gray']
        mix_node.inputs['Factor'].default_value = settings['tex_mix']
        diffuse_BSDF_node.inputs['Roughness'].default_value = settings['oren_rough']
        principled_BSDF_node.inputs['Roughness'].default_value = settings['princ_rough']
    ...
    def create_mars_opt_shader(
        ...
        if 'IOR' in principled_BSDF_node.inputs:
            principled_BSDF_node.inputs['IOR'].default_value = 1.50
        for _sp in ('Specular IOR Level', 'Specular'):
            if _sp in principled_BSDF_node.inputs:
                principled_BSDF_node.inputs[_sp].default_value = 0.50
                break
        for _sl in ('Metallic', 'Clearcoat', 'Sheen', 'Coat Weight'):
            if _sl in principled_BSDF_node.inputs:
                principled_BSDF_node.inputs[_sl].default_value = 0.0
        value_node.outputs[0].default_value = settings['base_gray']
        mix_node.inputs['Factor'].default_value = settings['tex_mix']
        diffuse_BSDF_node.inputs['Roughness'].default_value = settings['oren_rough']
        principled_BSDF_node.inputs['Roughness'].default_value = settings['princ_rough']
    ...
    def assign_material_to_object(
        ...
        """Assign a material to a corto body object
    ...
    @staticmethod
    def save_material_nodetree(
        material: bpy.types.Material, 
        output_path: str
        ):
        """Serialize and save a material node tree with all properties
        
        Args:            
            material (bpy.data.materials): Material to serialize
            output_path (str): File path to save the JSON representation of the material node tree
        """
        ...
        def serialize_node(
            ...
            Args:
                node (bpy.types.Node): The Blender node to serialize
            """
    ...
    def load_material_nodetree(
        ...
        Args:
            material_name (str): Name of the material to load
            state (corto.State): CORTO state object, used for handling file paths and

        Return:
            material: the loaded material with the node tree reconstructed from the JSON file
        """
    ...
    def load_uv_data(
        ...
        Args:
            body (corto.Body): Body object to which the UV data will be applied
